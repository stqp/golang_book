...
func main() {
	setColorIndex, _ := strconv.Atoi(os.Args[1])
	lissajous(os.Stdout, setColorIndex)
}
func lissajous(out io.Writer, setColorIndex int) {
	const (
	...
	for i := 0; i < nframes; i++ {
		...
		img := image.NewPaletted(rect, palettes[setColorIndex])
